...
// v5 includes the Krajevi i geografija subpages.
const CACHE = 'patriasoul-shell-v5';
...
self.addEventListener('fetch', event => {
  ...
  event.respondWith(fetch(request).then(response => { if (response.ok) { const copy = response.clone(); caches.open(CACHE).then(cache => cache.put(request, copy)).catch(() => undefined); } return response; }).catch(() => caches.match(request).then(cached => cached || caches.match('/404.html'))));
});
