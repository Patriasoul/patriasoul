...
(function(){
  ...
  function ensureHomeNavigation(){
    ...
    var existing=nav.querySelector('.ps-nav-home');
    if(existing){
      if(existing!==nav.firstElementChild) nav.insertBefore(existing,nav.firstElementChild);
      return;
    }
    ...
    nav.insertBefore(wrap,nav.firstElementChild);
  }
  ...
  function watchHomeNavigation(){
    ...
    [100,300,700,1500,3000].forEach(function(ms){ setTimeout(ensureHomeNavigation,ms); });
  }
  ...
  function renderStats(){
    ...
    target.innerHTML=['<div class="portal-stat"><strong>'+esc(cities || 127)+'</strong><span>gradova u registru</span></div>','<div class="portal-stat"><strong>'+esc(questions || 2000)+'+</strong><span>kviz pitanja</span></div>','<div class="portal-stat"><strong>'+esc(monuments || '—')+'</strong><span>memorijalnih zapisa</span></div>','<div class="portal-stat"><strong>1</strong><span>mjesto istine</span></div>'].join('');
  }
  ...
  function init(){ watchHomeNavigation(); renderStats(); renderTimeline(); document.documentElement.classList.add('portal-home-ready'); }
