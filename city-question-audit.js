...
// Kontrolira 127 kanonskih gradova, 127 slojeva, 75 pitanja po gradu,
...
(function(global){'use strict';
  const TARGET_CITIES=127, TARGET_LAYERS=127, TARGET_PER_CITY=75, TARGET_TOTAL=9525;
  ...
  function run(){
    ...
    const loadedLayers=[];
    for(let i=1;i<=TARGET_LAYERS;i++)if(global[`PatriaCityVerified${i}`]?.forCity)loadedLayers.push(i);
    const missingLayers=[];for(let i=1;i<=TARGET_LAYERS;i++)if(!loadedLayers.includes(i))missingLayers.push(i);
