...
function isoDate(date=new Date()){const d=new Date(date);return `${d.getFullYear()}-${String(d.getMonth()+1).padStart(2,'0')}-${String(d.getDate()).padStart(2,'0')}`;}
...
function translateLiturgicalName(name){const value=String(name||'').trim();if(!value)return '';const match=value.match(/^(Sunday|Monday|Tuesday|Wednesday|Thursday|Friday|Saturday) of the (first|second|third|fourth|fifth|sixth|seventh|eighth|ninth|tenth|eleventh|twelfth|thirteenth|fourteenth|fifteenth|sixteenth|seventeenth|eighteenth|nineteenth|twentieth|twenty-first|twenty-second|twenty-third|twenty-fourth|twenty-fifth|twenty-sixth|twenty-seventh|twenty-eighth|twenty-ninth|thirtieth|thirty-first|thirty-second|thirty-third|thirty-fourth) week of Ordinary Time$/i);if(match){const weekday={Sunday:'Nedjelja',Monday:'Ponedjeljak',Tuesday:'Utorak',Wednesday:'Srijeda',Thursday:'Četvrtak',Friday:'Petak',Saturday:'Subota'}[match[1]];const ordinal=ORDINALS[match[2].toLowerCase()]||match[2];return `${weekday} ${ordinal} tjedna kroz godinu`;}return value.replace(/^Saint\s+/i,'Sv. ').replace(/^Blessed\s+/i,'Bl. ')}
