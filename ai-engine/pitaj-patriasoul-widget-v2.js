// PatriaSoul — global "Pitaj PatriaSoul" widget v2
(function () {
  'use strict';
  if (window.__PATRIA_ASK_WIDGET_V2__) return;
  window.__PATRIA_ASK_WIDGET_V2__ = true;
  ...
  async function waitForEngine() {
    ...
    if (window.PatriaSoulAIReady && typeof window.PatriaSoulAIReady.then === 'function') {
      try { await window.PatriaSoulAIReady; } catch (_) {}
    }
  ...
  async function askQuestion(q) {
    ...
    try {
      ...
      if (!agent) {
        ...
        throw new Error(status.error || 'AI lanac se nije uspio pokrenuti.');
      }
