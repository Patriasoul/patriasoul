...
function loadLatestVerifiedLayer(){if(typeof document==='undefined')return;for(let i=36;i<=56;i++){const src='/patriasoul-city-questions-verified-'+i+'.js';if(document.querySelector('script[src="'+src+'"]'))continue;const s=document.createElement('script');s.src=src;s.defer=false;document.head.appendChild(s)}}
loadLatestVerifiedLayer();
function verifiedLayers(city){const out=[];for(let i=0;i<=56;i++){const layer=global[`PatriaCityVerified${i||''}`];if(layer?.forCity)out.push((layer.forCity(city)||[]).slice(0,75))}return out}
