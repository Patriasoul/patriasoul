...
function verifiedLayers(city){const out=[];for(let i=0;i<30;i++){const layer=global[`PatriaCityVerified${i||''}`];if(layer?.forCity)out.push(layer.forCity(city)||[])}return out}
...
function start(city){const key=cityKey(city),unique=verifiedCityPool(city);if(unique.length!==75)return [];const h=history();const seed=(Date.now()^((key.length*2654435761)>>>0))>>>0;const selected=global.PatriaQuiz.seededShuffle(unique,seed);h[key]=selected.map(q=>String(q.id));saveHistory(h);return selected.map(q=>global.PatriaQuiz.prepare(q))}
