...
function loadLatestVerifiedLayer(){if(typeof document==='undefined')return;const src='/patriasoul-city-questions-verified-36.js';if(document.querySelector('script[src="'+src+'"]'))return;const s=document.createElement('script');s.src=src;s.defer=false;document.head.appendChild(s)}
loadLatestVerifiedLayer();
function verifiedLayers(city){const out=[];for(let i=0;i<=36;i++){const layer=global[`PatriaCityVerified${i||''}`];if(layer?.forCity)out.push(layer.forCity(city)||[])}return out}
