...
function verifiedCityPool(city){const key=cityKey(city),cityBank=global.PatriaCityQuestions?.forCity?.(city)||[];const combined=[cityBank,...verifiedLayers(city)].flat();return Array.from(new Map(combined.map(q=>[String(q.id),q]).values()).values()).filter(q=>q&&q.cityId===key&&q.citySource==='verified'&&q.quality==='verified')}
