// PatriaSoul — Brani svoj grad: jedan grad = kompletnih 75 pitanja.
...
function cityKey(city){return String(city).toLocaleLowerCase().normalize('NFD').replace(/[\u0300-\u036f]/g,'').replace(/đ/g,'d').replace(/[^a-z0-9]+/g,'-').replace(/^-|-$/g,'')}
function verifiedLayers(city){const out=[];for(let i=0;i<35;i++){const layer=global[`PatriaCityVerified${i||''}`];if(layer?.forCity)out.push(layer.forCity(city)||[])}return out}
function verifiedCityPool(city){const key=cityKey(city),base=global.PatriaCityQuestions?.forCity?.(city)||[],combined=[base,...verifiedLayers(city)].flat();return Array.from(new Map(combined.map(q=>[String(q.id),q])).values()).filter(q=>q&&q.cityId===key&&q.citySource!=='registry-complete')}
function start(city){const key=cityKey(city),unique=verifiedCityPool(city);if(unique.length!==75)return [];const seed=(Date.now()^((key.length*2654435761)>>>0))>>>0;const selected=global.PatriaQuiz.seededShuffle(unique,seed);const h=history();h[key]=selected.map(q=>String(q.id));saveHistory(h);return selected.map(q=>global.PatriaQuiz.prepare(q))}
function finish(city,score,correct,answersCount){const total=Number(answersCount)||75,p=global.PatriaPlayer.current(),result={id:crypto.randomUUID?.()||Date.now()+'',city,score:Math.max(0,score|0),correct:Math.max(0,correct|0),answers:total,name:p.name||'Gost',date:new Date().toISOString(),period:'daily'};save(read().concat(result));global.PatriaPlayer.addCityScore(city,result.score);global.PatriaPlayer.recordResult({category:'brani-svoj-grad',points:result.score,xp:result.score,correct:result.correct,answers:total,city,period:'daily',id:result.id});const m=missions();m.played++;m.points+=result.score;if(result.correct===total)m.perfect++;saveMissions(m);global.PatriaBadges?.evaluate(global.PatriaPlayer.current());return{...result,missions:{played:m.played,perfect:m.perfect,points:m.points,completed:[m.played>=1?'prvi-izazov':null,m.perfect>=1?'bezgresni-branitelj':null,m.points>=1000?'cuvar-grada':null].filter(Boolean)}}}
