...
(function(){
  ...
  if(categories.length!==10||categories.some(k=>byCategory[k].length!==100))throw new Error('PatriaSoul: ocekuje se tocno 10 kategorija po 100 pitanja. '+JSON.stringify({categories,counts}));
  ...
  function validate(){const ids=new Set(),by={},questionsByText=new Set();let invalid=0,duplicates=0,duplicateQuestions=0;bank.forEach(q=>{if(ids.has(q.id))duplicates++;ids.add(q.id);const text=q.question.toLocaleLowerCase('hr-HR');if(questionsByText.has(text))duplicateQuestions++;questionsByText.add(text);if(!q.id||!q.question||q.answers.length!==4||q.answers.some(a=>!a.trim())||q.correctIndex<0||q.correctIndex>3)invalid++;by[q.category]=(by[q.category]||0)+1;});return{total:bank.length,categories:categories.length,invalid,duplicates,duplicateQuestions,exact1000:bank.length===1000&&categories.length===10&&invalid===0&&duplicates===0&&duplicateQuestions===0,byCategory:by,eachCategory100:categories.every(k=>by[k]===100)};}
