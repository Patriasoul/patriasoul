...
// Mijesani kviz je nacin igre nad svih 1000 pitanja, nije 11. kategorija.
(function(){
  'use strict';
  const KNOWN_LABELS={
    povijest:'Hrvatska povijest',domovinski_rat:'Domovinski rat',kraljevi:'Hrvatski kraljevi',
    katolicka_bastina:'Katolicka bastina',kulturna_bastina:'Kulturna bastina',geografija:'Hrvatska geografija',
    glazba:'Hrvatska glazba',gradovi:'Gradovi i znamenitosti',jezik:'Jezik i knjizevnost'
  };
  const humanize=k=>KNOWN_LABELS[k]||k.replace(/_/g,' ').replace(/\b\w/g,c=>c.toUpperCase());
  window.PATRIA_QUESTIONS=[];
  window.PATRIA_CATEGORIES={};

  // Sve banke su u korijenu repozitorija. Ne ucitavamo data.js kao dodatnu bazu,
  // jer cilj ove verzije je strogo 10 × 100 = 1000 pitanja.
  ...
  Object.keys(extra).forEach(key=>{
    ...
    if(Array.isArray(extra[key]))byCategory[key]=extra[key].map((q,i)=>normalize(q,key,i,key));
  });
  ...
  const counts=Object.fromEntries(categories.map(k=>[k,byCategory[k].length]));
  if(categories.length!==10||categories.some(k=>byCategory[k].length!==100)){
    throw new Error('PatriaSoul: ocekuje se tocno 10 kategorija po 100 pitanja. '+JSON.stringify({categories,counts}));
  }

  window.PATRIA_CATEGORIES=Object.fromEntries(categories.map(k=>[k,humanize(k)]));
  const bank=categories.flatMap(k=>byCategory[k]);
  window.PATRIA_QUESTIONS=bank;
  ...
  function validate(){
    const ids=new Set(),by={};let invalid=0,duplicates=0;
    bank.forEach(q=>{if(ids.has(q.id))duplicates++;ids.add(q.id);if(!q.id||!q.question||q.answers.length!==4||q.answers.some(a=>!a.trim())||q.correctIndex<0||q.correctIndex>3)invalid++;by[q.category]=(by[q.category]||0)+1;});
    return {total:bank.length,categories:categories.length,invalid,duplicates,exact1000:bank.length===1000&&categories.length===10&&invalid===0&&duplicates===0,byCategory:by,eachCategory100:categories.every(k=>by[k]===100)};
  }
  window.PatriaQuiz={
    ready:Promise.resolve(),bank:()=>window.PATRIA_QUESTIONS||[],categories:()=>categories.slice(),shuffle,prepare,
    byCategory:(category,count=10)=>{const list=category==='mijesani'?bank:bank.filter(q=>q.category===category);return shuffle(list).slice(0,count).map(prepare);},
    periodSeed,seededShuffle,
    periodQuiz:(period='daily',count=10)=>seededShuffle(bank,periodSeed(period)).slice(0,count).map(prepare),
    periodCategoryQuiz:(period='daily',category='mijesani',count=10)=>{const list=category==='mijesani'?bank:bank.filter(q=>q.category===category);return seededShuffle(list,periodSeed(period)).slice(0,count).map(prepare);},
    validate
  };
